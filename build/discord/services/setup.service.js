...
import CreateRoleCapitaoIfNotExistsService from "./create-role-capitao.service.js";
export default async function SetupService({ methods }) {
    const { messageStack } = methods;
    await messageStack.push("🚀 Iniciando a configuração do servidor. Isso pode levar alguns minutos...");
    ...
    try {
        ...
        await messageStack.push(`❌ Falha ao configurar o cargo de Capitão: ${error instanceof Error ? error.message : "Erro desconhecido"}`);
    ...
    const createdModalityRoles = [];
    const modalityKeys = Object.keys(modalidadesDefinition);
    for (const key of modalityKeys) {
        try {
            await messageStack.push(`⏳ Verificando/criando papel para modalidade '${key}'...`);
            ...
            await messageStack.push(`❌ ${errorMessage}: ${error instanceof Error ? error.message : "Erro desconhecido"}`);
            await methods.editReply(messageStack.getCurrentMessageBody());
    ...
    logger.log("Iniciando a criação das categorias para os canais de voz dos times." //
    );
    for (const role of createdModalityRoles) {
        // Uses the sequentially created roles
        try {
            await messageStack.push(`⏳ Verificando/criando categoria para times de '${role.name}'...`);
            ...
            await messageStack.push(`✅ Categoria 'Times de ${role.name}' configurada.`);
        }
        catch (categoryError) {
            const errorMessage = `Erro ao assegurar categoria para '${role.name}'`;
            logger.error(errorMessage + ":", categoryError);
            await messageStack.push(`❌ ${errorMessage}: ${categoryError instanceof Error
    ...
    await messageStack.push("👍 Categorias para os canais de voz dos times criadas com sucesso!");
    logger.success("Categorias para os canais de voz dos times criadas com sucesso." //
    );
    // 4. Persist Modalities in Database
    await messageStack.push("\nℹ️ Iniciando persistência das modalidades no banco de dados...");
    logger.log("Iniciando a persistência/atualização das modalidades no banco de dados." //
    );
    for (const role of createdModalityRoles) {
        try {
            await messageStack.push(`💾 Registrando modalidade '${role.name}' no banco de dados...`);
            ...
            logger.success(`Modalidade '${role.name}' (Role ID: ${role.id}) persistida/atualizada no banco de dados.` //
            );
            await messageStack.push(`✅ Modalidade '${role.name}' (Role ID: ${role.id}) registrada.`);
        }
        catch (dbProcessingError) {
            const errorMessage = `Erro ao processar a modalidade '${role.name}' no banco de dados`;
            logger.error(errorMessage + ":", //
            dbProcessingError);
            await messageStack.push(`❌ ${errorMessage}: ${dbProcessingError instanceof Error
    ...
    await messageStack.push("👍 Modalidades persistidas/atualizadas no banco de dados com sucesso!");
    logger.success("Modalidades persistidas/atualizadas no banco de dados com sucesso." //
    );
