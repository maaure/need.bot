...
export default async function AdicionarJogadorService(methods) {
    ...
    if (!methods.member.permissions.has(PermissionFlagsBits.Administrator)) {
        ...
        if (!existingPlayer)
            throw new Error("Houve um erro inesperado durante o cadastro do novo membro.");
        ...
        if (!participation)
            throw new Error("Você não pode adicionar um jogador a um time que você não faz parte.");
    }
    else {
    ...
    logger.log(`Iniciando o processo de adição do jogador ${playerName} (Discord ID: ${guildMemberId}) ao time ${teamName}.`);
    ...
    logger.success(`Jogador ${playerEntity.name} (Discord ID: ${guildMemberId}) processado/assegurado no banco.`);
    ...
    if (participacaoMesmaModalidade) {
        throw new Error(`O jogador ${playerEntity.name} já está no time "${participacaoMesmaModalidade.team.name}" da modalidade "${teamWithModality.modality.name}".`);
    }
